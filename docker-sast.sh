...
if [[ -z "$no_shellcheck" ]]; then
  ...
  if [[ $target_type == "directory" ]]; then
    for f in "$target"/**/*.sh; do
       ...
       [[ -e "$f" ]] || continue
       shellcheck "$f" --shell=bash || echo "I FAILED"
    done
  elif [[ "${target: -3}" == ".sh" ]]; then
    shellcheck "$target" || echo "I FAILED"
  fi
fi
...
if [[ -z "$no_yamllint" ]]; then
  ...
  if [[ $target_type == "directory" || "${target: -5}" == ".yaml" ]]; then
      ...
      yamllint "$target" -d "{extends: default, rules: {line-length: {max: 120}}}" || echo "I FAILED"
  fi
fi
...
if [[ -z "$no_jsonlint" ]]; then
  ...
  if [[ $target_type == "directory" ]]; then
    for f in "$target"/**/*.json; do
      # if glob does not match, stop execution
      [[ -e "$f" ]] || continue
      jsonlint -q "$f" || echo "Error in file: $f" && echo "I FAILED";
    done
  elif [[ "${target: -5}" == ".json" ]]; then
    jsonlint -q "$target" || echo "I FAILED"
  fi
fi
...
if [[ -n "$trufflehog_target" ]]; then
  printf  ">> trufflehog...\n"
  trufflehog --cleanup --entropy=true "$trufflehog_target" || echo "I FAILED"
fi
...
############################# Bandit #####################################
  ...
  if [[ -z "$no_bandit" ]]; then
    printf ">> bandit...\n"
    if [[ $target_type == "directory" ]]; then
      bandit -r -q -l "$target" || echo "I FAILED"
    elif [[ "${target: -3}" == ".py" ]]; then
      bandit -q -l "$target" || echo "I FAILED"
    fi
  fi
...
############################# Flake8 #####################################
  if [[ -z "$no_flake8" ]]; then
    ...
    if [[ $target_type == "directory" || "${target: -3}" == ".py" ]]; then
      flake8 --max-line-length=139 "$target" || echo "I FAILED"
    fi
  fi
fi
...
############################# TSLint #####################################
  if [[ -z "$no_tslint" ]]; then
    printf ">> tslint...\n"
    tslint --init || echo "I FAILED"
    if [[ $target_type == "directory" ]]; then
      tslint "$target"/**/*.ts || echo "I FAILED"
    elif [[ "${target: -3}" == ".ts" ]]; then
      tslint "$target" || echo "I FAILED"
    fi
  fi
